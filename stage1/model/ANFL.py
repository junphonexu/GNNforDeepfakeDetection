...
class GNN(nn.Module):
    def __init__(self, in_channels, num_classes, neighbor_num=2, metric='dots'):
        super(GNN, self).__init__()
        # in_channels:节点特征尺寸 dim of node feature
        # num_classes:节点数量 num of nodes
        # neighbor_num: K in paper and we select the top-K nearest neighbors for each node feature.
        # metric:用于评估节点相似度 metric for assessing node similarity.
...
class MEFARG(nn.Module):
    def __init__(self, num_classes=12, backbone='resnet', neighbor_num=4, metric='dots'):
